...
class RandomCrop(object):
    ...
    def __call__(self, results):
        ...
        Args:
            img (ndarray): Image to be cropped.
        """
        for key in results.get('img_fields', ['img_SAR','img_EO']):
            img = results[key]
...
class PaddingCenterCrop(object):
    ...
    def __init__(self, crop_size, padding=None,padding_mode='edge'):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img = mmcv.impad(
                img, padding=self.padding,padding_mode=self.padding_mode,pad_val=(50,34,8))
...
class Normalize(object):
    ...
    def __init__(self, mean_EO, std_EO, mean_SAR, std_SAR, to_rgb=True):
        self.mean_EO = np.array(mean_EO, dtype=np.float32)
        self.std_EO = np.array(std_EO, dtype=np.float32)
        self.mean_SAR = np.array(mean_SAR, dtype=np.float32)
        self.std_SAR = np.array(std_SAR, dtype=np.float32)
        self.to_rgb = to_rgb

    def __call__(self, results):
        for key in results.get('img_fields', ['img_EO','img_SAR']):
            if key == 'img_EO':                
                results[key] = mmcv.imnormalize(results[key], self.mean_EO, self.std_EO,
                                                self.to_rgb)
            elif key == 'img_SAR': 
                results[key] = mmcv.imnormalize(results[key], self.mean_SAR, self.std_SAR,
                                                self.to_rgb)                
        results['img_norm_cfg'] = dict(
            mean_EO=self.mean_EO, std_EO=self.std_EO, mean_SAR=self.mean_SAR, std_SAR=self.std_SAR,to_rgb=self.to_rgb)
        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(mean_SAR={list(self.mean_SAR)}, '
        repr_str += f'std_SAR={list(self.std_SAR)}, '
        repr_str += f'(mean_EO={list(self.mean_EO)}, '
        repr_str += f'std_EO={list(self.std_EO)}, '
...
class Albu_dualflow(object):
    ...
    def __init__(self, transforms, keymap=None, additional_targets=None,update_pad_shape=False):
        if Compose is None:
            raise RuntimeError('albumentations is not installed')
        ...
        if not keymap:
            ...
            self.keymap_to_albu = keymap
        self.keymap_back = [{v: k for k, v in keymap_to_albu.items()} for keymap_to_albu in self.keymap_to_albu] 
    ...
    def __call__(self, results):
        ...
        for i in range(len(self.keymap_to_albu)):
	        results = self.mapper(results, self.keymap_to_albu[i])
	        ...
	        # back to the original format
	        results = self.mapper(results, self.keymap_back[i])

        # update final shape
        if self.update_pad_shape:
            results['pad_shape'] = results['img_EO'].shape
