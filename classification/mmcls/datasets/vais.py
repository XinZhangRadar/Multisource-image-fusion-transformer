...
class VAIS(Dataset, metaclass=ABCMeta):
    """Base dataset.
    ...
    def __init__(self, data_prefix, pipeline, ann_file=None, test_mode=False):
        super(VAIS, self).__init__()
        self.CLASSES = {'cargo':0,'medium-other':1,'passenger':2,'sailing':3,'small':4,'tug':5}
    ...
    def load_annotations(self):
        ...
        data_infos = []
        for EO_Files,IR_Files,FG_Labels,basic_Labels,fID, isTrain,isNight in self.samples:
            if EO_Files == 'null' or IR_Files == 'null':
                continue
            if self.test_mode:
                if isTrain == '1':
                    continue
            ...
            info['img_info'] = {'filename_EO': EO_Files,'filename_SAR':IR_Files}
            info['gt_label'] = np.array(self.CLASSES[basic_Labels], dtype=np.int64)
