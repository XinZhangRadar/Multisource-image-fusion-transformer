...
class Attention(nn.Module):
    ...
    def show_map_on_image(self, attmaps):
        #import pdb;pdb.set_trace()

        img = cv2.imread("/home/zhangxin/mmclassification/vis/image_EO.jpg") #######
        ...
        beta = 0.5
        gamma = 0
        h = w  = int(np.sqrt(attmaps.shape[0]))
        position_map = np.zeros((h,w))


        clas_token_atte = (attmaps[0][1:].reshape(h,w)>0.0051)*255


        clas_token_atte = clas_token_atte.astype(np.uint8)
        

        clas_token_atte = cv2.resize(clas_token_atte, (img.shape[1], img.shape[0]))
        clas_token_atte = np.tile(clas_token_atte.reshape(clas_token_atte.shape[0],clas_token_atte.shape[1],1),[1,1,3])
        mask_img = cv2.addWeighted(img, alpha, clas_token_atte*1.0, beta, gamma)
        cv2.imwrite('/home/zhangxin/mmclassification/attention_map_self/attmap/attenmap.jpg',mask_img)
        ...
        for j in range(h):
            for i in range(w):
                ...
                position_map[j,i] = 255
                cv2.imwrite("/home/zhangxin/mmclassification/attention_map_self/posmap/"+str(j)+'_'+str(i)+'_position.jpg',position_map)
                position_map*= 0
                attmap = (attmaps[j*w+i].reshape(h,w)>0.0051)*255
                attmap = attmap.astype(np.uint8)
                # attmap_color = cv2.applyColorMap(attmap[j*w+i].reshape(h,w).detach().cpu().numpy().astype(np.uint8), cv2.COLORMAP_JET)
                # cv2.imwrite("/home/zhangxin/mmsegmentation/attention_map/attmap/"+str(j)+'_'+str(i)+'_attenmap.jpg',attmap_color)
                #attmap = attmap/np.max(attmap)
                attmap = cv2.resize(attmap, (img.shape[1], img.shape[0]))
                attmap = np.tile(attmap.reshape(attmap.shape[0],attmap.shape[1],1),[1,1,3])
                mask_img = cv2.addWeighted(img, alpha, attmap*1.0, beta, gamma)
                cv2.imwrite("/home/zhangxin/mmclassification/attention_map_self/attmap/"+str(j)+'_'+str(i)+'_attenmap.jpg',mask_img)
        return mask_img 
    def forward(self, x, mask = None):
        ...
        import pdb;pdb.set_trace()
        #self.show_map_on_image(attn[0].mean(0).detach().cpu().numpy())

        #self.show_map_on_image(dots[0][:,:,1:][:,1:,:].mean(0)*10000)
        #import pdb;pdb.set_trace()
...
class MS_Attention(nn.Module):
    ...
    def show_map_on_image(self, attmaps):
        ...
        clas_token_atte = np.tile(clas_token_atte.reshape(clas_token_atte.shape[0],clas_token_atte.shape[1],1),[1,1,3])
        mask_img = cv2.addWeighted(img, alpha, clas_token_atte*1.0, beta, gamma)
        cv2.imwrite('/home/zhangxin/mmclassification/attention_map_cross/attmap/attenmap.jpg',mask_img)
        ...
        for j in range(h):
            for i in range(w):
                ...
                position_map[j,i] = 255
                cv2.imwrite("/home/zhangxin/mmclassification/attention_map_cross/posmap/"+str(j)+'_'+str(i)+'_position.jpg',position_map)
                position_map*= 0
                attmap = (attmaps[j*w+i].reshape(h,w)>0.0051)*255
                attmap = attmap.astype(np.uint8)
                # attmap_color = cv2.applyColorMap(attmap[j*w+i].reshape(h,w).detach().cpu().numpy().astype(np.uint8), cv2.COLORMAP_JET)
                ...
                attmap = np.tile(attmap.reshape(attmap.shape[0],attmap.shape[1],1),[1,1,3])
                mask_img = cv2.addWeighted(img, alpha, attmap*1.0, beta, gamma)
                cv2.imwrite("/home/zhangxin/mmclassification/attention_map_cross/attmap/"+str(j)+'_'+str(i)+'_attenmap.jpg',mask_img)
        return mask_img 

    def forward(self, x_A,x_B, mask = None, if_vis = False):
        #import pdb;pdb.set_trace()
...
class FPN_MSFT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0., in_channels=[256, 512, 1024, 2048],out_channels=256,num_outs=5,combine='add',loss_out = 2):
        super().__init__()
        #share mate
    ...
    def forward(self, fea_A, fea_B, mask = None):
        ...
        if self.loss_out ==2:
            if self.combine =='add':
                ...
                return self.mlp_head(torch.cat((cls_tokens_A_encoder,cls_tokens_B_encoder),1)),self.mlp_head(torch.cat((cls_tokens_A_decoder,cls_tokens_B_decoder),1))
        elif self.loss_out ==3:
            if self.combine =='add':
                return self.tl_head_A(cls_tokens_A.squeeze(1)), self.tl_head_B(cls_tokens_B.squeeze(1)),self.mlp_head(cls_tokens_A_encoder+cls_tokens_B_encoder),self.mlp_head(cls_tokens_A_decoder+cls_tokens_B_decoder)
            elif self.combine =='concat':
                return self.tl_head_A(cls_tokens_A.squeeze(1)), self.tl_head_B(cls_tokens_B.squeeze(1)),self.mlp_head(torch.cat((cls_tokens_A_encoder,cls_tokens_B_encoder),1)),self.mlp_head(torch.cat((cls_tokens_A_decoder,cls_tokens_B_decoder),1))            
