...
class MSFT(nn.Module):
    ...
    def forward(self, fea_A, fea_B, mask = None,loss_type = None):
        ...
        x_A_E = x_A + self.pos_embedding[:, :(n)]
        x_A_E = self.dropout(x_A_E)

        # x_B = torch.cat((cls_tokens_B, x_B), dim=1)
        x_B_E = x_B + self.pos_embedding[:, :(n)]
        x_B_E = self.dropout(x_B_E)
        ...
        x_A_D,x_B_D = self.dual_decoder(x_A_E,x_B_E, mask)
        ...
        if loss_type == 'aux':
            ...
            return (out_cnn_encoder,out_trans_encoder,out_trans_decoder)
